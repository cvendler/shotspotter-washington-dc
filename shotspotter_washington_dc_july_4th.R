...
# "washington_dc_july_4th_data"

washington_dc_july_4th_data <- read_csv(url("http://justicetechlab.org/wp-content/uploads/2018/05/washington_dc_2006to2017.csv"), 
  ...
  # "year", "month", and "day" separated by a hyphen (like "2014-07-04")
  
  mutate(date = paste(year, month, day, sep = "-")) %>% 
  
  # Create a new variable called "date_hour" by pasting together the values of
  # "date" and "hour", separated by " Hour " (like "2014-07-04 Hour 23")
  
  mutate(date_hour = paste(date, hour, sep = " Hour ")) %>% 
  
  # Filter the data to keep only observations where "date" is either
  # "2014-07-04" or "2014-07-05"; I choose the span of these two days, as
  # fireworks celebrations typically happen on the midnight of July 4th,
  # bleeding into July 5th
  
  filter(date %in% c("2014-07-04", "2014-07-05")) %>% 
...
# Use "st_as_sf" to turn the "washington_dc_july_4th_data" dataset into the
...
# "washington_dc_july_4th_locations"

washington_dc_july_4th_locations <- st_as_sf(washington_dc_july_4th_data, 
                                             coords = c("longitude", "latitude"), 
                                             crs = st_crs(washington_dc_shape))


# Create a ggplot animation using the "washington_dc_shape" data; store as
# "washington_dc_july_4th_animation" in order to be able to save the animation
# as a gif below

washington_dc_july_4th_animation <- ggplot(data = washington_dc_shape) + 
  ...
  # Call "geom_sf" on the "washington_dc_july_4th_locations" dataset so that the
  # locations of shots appear as points on the map; color by the variable "type"
  # so that "Single Gunshot", "Multiple Gunshots", and "Gunshot or Firecracker"
  # shots appear in different colors; set the legend to "point" to reflect the
  # points on the animation more accurately (to match them)
  
  geom_sf(data = washington_dc_july_4th_locations, aes(color = type), 
          show.legend = "point") + 
...
geom_segment(aes(x = -77.0365, y = 38.8977, xend = -77.08, yend = 38.8977), 
  ...
  labs(title = "Locations and Types of Shots (Gunshots and Firecrackers)\nin Washington, D.C. on: {closest_state}", 
       subtitle = "On the midnight of July 4th, the celebration of American independence,\nthere is a dramatic spike in the number of gunshots and firecrackers", 
  ...
  # hour on the days 2014-07-04 to 2014-07-05
  
  transition_states(date_hour)


# Save the animation "washington_dc_july_4th_animation" as a gif, called
# "washington_dc_july_4th_animation.gif" in the folder
# "shotspotter_washington_dc_app"

anim_save("shotspotter_washington_dc_app/washington_dc_july_4th_animation.gif", animate(washington_dc_july_4th_animation))
